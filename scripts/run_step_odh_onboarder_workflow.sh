...
#
# Pass --release to run the optional Release onboarder (step key: onboarder_release).
# Without --release, always runs the CI onboarder (step key: onboarder_workflow).
#
# Exit codes:
...
#   2  Step not applicable (skipped) — writes pipeline_state.json (status=skipped)
#
...
if [[ "$RELEASE_MODE" == "true" ]]; then
  STEP_KEY="onboarder_release"
  LABEL_RAISED="tekton-release-pr-raised"
else
  STEP_KEY="onboarder_workflow"
...
EXISTING_URL=$(jq -r --arg k "$STEP_KEY" '.steps[$k].pr_url // ""' "$PIPELINE_STATE")
...
BUILD_TYPE_YAML=$(grep -m1 'build_type:'       "$YAML_FILE" | awk '{print $2}' 2>/dev/null || echo "CI")
VERSION=$(grep -m1        'odh_release_tag:'  "$YAML_FILE" | awk '{print $2}' 2>/dev/null || echo "")
RELEASE_BRANCH=$(grep -m1 'odh_release_branch:' "$YAML_FILE" | awk '{print $2}' 2>/dev/null || echo "")

REPO_NAME="${REPO_URL##*/}"; REPO_NAME="${REPO_NAME%.git}"
COMPONENT="$REPO_NAME"

BUILD_TYPE_YAML_LOWER="${BUILD_TYPE_YAML,,}"
if [[ "$BUILD_TYPE_YAML_LOWER" == "release" ]]; then
  BUILD_TYPE_YAML="Release"
else
  BUILD_TYPE_YAML="CI"
fi
...
if [[ "$RELEASE_MODE" == "true" ]]; then
  if [[ -z "$VERSION" ]]; then
    echo "No odh_release_tag in YAML — skipping optional Release onboarder."
    bash "$SCRIPTS_DIR/update_pipeline_state.sh" \
      --state "$PIPELINE_STATE" --step "$STEP_KEY" --status skipped
    exit 2
  fi
  # Legacy tickets ran Release in onboarder_workflow before this step existed.
  if [[ "$BUILD_TYPE_YAML" == "Release" ]]; then
    WF_STATUS=$(jq -r '.steps.onboarder_workflow.status // "pending"' "$PIPELINE_STATE")
    if [[ "$WF_STATUS" == "done" || "$WF_STATUS" == "merged" || "$WF_STATUS" == "pr_raised" ]]; then
      echo "Legacy Release onboarder already ran in onboarder_workflow — skipping onboarder_release."
      bash "$SCRIPTS_DIR/update_pipeline_state.sh" \
        --state "$PIPELINE_STATE" --step "$STEP_KEY" --status skipped
      exit 2
    fi
  ...
  BUILD_TYPE="Release"
  WORKFLOW_BRANCH="${RELEASE_BRANCH:-stable}"
else
  BUILD_TYPE="CI"
...
if [[ "${ONBOARD_DRY_RUN:-false}" == "true" ]]; then
  echo "ONBOARD_DRY_RUN=true — skipping onboarder workflow trigger, marking ${STEP_KEY} as done."
  uv run --script "$SCRIPTS_DIR/update_jira_issue.py" "$JIRA_URL" \
    --add-label "$LABEL_RAISED" \
    --comment "[step:${STEP_KEY}] Skipped (ONBOARD_DRY_RUN=true). ODH onboarder workflow not triggered." || true
  bash "$SCRIPTS_DIR/update_pipeline_state.sh" \
    --state "$PIPELINE_STATE" --step "$STEP_KEY" \
    --status done
  exit 0
fi
...
WORKFLOW_INPUTS=(
  ...
  "--input" "pr_target_branch=$WORKFLOW_BRANCH"
...
uv run --script "$SCRIPTS_DIR/update_jira_issue.py" "$JIRA_URL" \
  --comment "[step:${STEP_KEY}] odh-konflux-onboarder workflow completed.
