...
RHOAI tickets must include `architectures`, `target_rhoai_version` (canonical form, e.g. `3.4` or `3.4-ea-2`), and `release_category` (`Generally Available` or `Tech Preview`) in addition to the common required fields. ODH tickets must include `build_type` (always `CI` for new tickets). When `odh_release_tag` is set, the orchestrator also runs the optional Release onboarder. Legacy tickets may use `build_type: Release` with `odh_release_tag` instead.
